...
public class FrightsDelightGUI {
    ...
    public static void renderNegativeHungerOverlay(HungerType hungerType, Player player, Minecraft mc, PoseStack poseStack, int left, int top) {
        ...
        for (int j = 0; j < 10; ++j) {
            ...
            mc.gui.blit(poseStack, x, y, hungerType.getOutlineUOffset(), hungerType.getOutlineVOffset(), 9, 9);

            if (hungerType.isHasFill()) {
                float effectiveHungerOfBar = (stats.getFoodLevel()) / 2.0F - j;

                if (effectiveHungerOfBar >= 1) {
                    mc.gui.blit(poseStack, x, y, hungerType.getOutlineUOffset() + hungerType.getUFillOffset(false), 0, 9, 9);
                } else if (effectiveHungerOfBar >= .5) {
                    mc.gui.blit(poseStack, x, y, hungerType.getOutlineUOffset() + hungerType.getUFillOffset(true), 0, 9, 9);
                }
            }
        }
