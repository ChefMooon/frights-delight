...
public class UndeadHungerOverlay {
    public static void renderUndeadHungerOverlay(Minecraft mc, PoseStack poseStack, int foodIconsOffset) {
        if (!getConfig()) return;
        ...
        if (player.getEffect(FrightsDelightEffects.UNDEAD_HUNGER.get()) != null) {
            FrightsDelightGUI.renderNegativeHungerOverlay(FrightsDelightGUI.HungerType.UNDEAD_HUNGER, player, mc, poseStack, left, top);
        }
    }
